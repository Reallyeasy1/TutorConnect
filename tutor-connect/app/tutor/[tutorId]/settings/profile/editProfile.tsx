...
export const EditProfile: React.FC<EditProfileProps> = ({ tutor, checked, show }) => {
	...
	const [contactNumber, setContactNumber] = useState(String(tutor.contactNumber));
	const [dateOfBirth, setDateOfBirth] = useState<Date | undefined>(new Date(tutor.dateOfBirth));
	const [age, setAge] = useState(tutor.age);
	const [nationality, setNationality] = useState(tutor.nationality);
	const [gender, setGender] = useState(tutor.gender);
	const [race, setRace] = useState(tutor.race);
	const [yearsOfExperience, setYearsOfExperience] = useState(tutor.yearsOfExperience);
	const [typeOfTutor, setTypeofTutor] = useState(tutor.typeOfTutor);
	const [highestEducationLevel, setHighestEducationLevel] = useState(tutor.highestEducationLevel);
	const [image, setImage] = useState(tutor.image);
	const [newImage, setNewImage] = useState<File | null>(null);
	const [preview, setPreview] = useState<string | null>(null);
	const [showSubjects, setShowSubjects] = useState(show);
	const [checkedSubjects, setCheckedSubjects] = useState<CheckedSubjects>(checked);
								...
								<div className="space-y-2">
									...
									<div className="grid grid-cols-2 gap-4">
										<div className="col-span-1 space-y-1">
											...
											<Input required value={age} onChange={(e) => setAge(parseInt(e.target.value))} id="age" type="number" />
										</div>
										<div className="col-span-1 space-y-1">
									...
									<div className="space-y-1">
										...
										<div className="space-y-1">
											...
											<div style={questionStyle.count}>
												{!introduction || introduction.trim() === "" ? 0 : introduction.trim().split(/\s+/).length}/{maxWords}{" "}
												words
											</div>
										</div>
										<div className="space-y-1">
										...
										<div className="space-y-1">
											...
											/>
											<div style={questionStyle.count}>
												{!studentsResults || studentsResults.trim() === "" ? 0 : studentsResults.trim().split(/\s+/).length}/
												{maxWords} words
											</div>
										</div>
									</div>
